# Wyatt Henri Brown, VestBot 2023
# desk clock with news headers, the date and time, and the weather. All showing on a 16x2 LCD.
...
# Openweathermap has its own API, which requires a token, get yours at their website
api_OpenWeather_token = ""
# api_OpenWeather_yourCity : Search for your city in openweathermap.org, then put the <city,country> code here with
# no space in between.
...
def long_string(display, text='', num_line=2, num_cols=16, speed=0.23):
    """ 
...
# we set the variable for OWM, keeping it in its own section, so we don't need to start it everytime
    ...
    while True:
        try:
            ...
            disp_string_weatherInfo = str(round(api_OpenWeather_json['main']['feels_like'])) + "￟C - " + \
                                      api_OpenWeather_json['weather'][0]['description'] + "   |   " + str(
                round(api_OpenWeather_json['main']['temp_min'])) + "￟LO - " + str(
                round(api_OpenWeather_json['main']['temp_max'])) + "￟HI   |   " + str(
                round(api_OpenWeather_json['main']['humidity'])) + "%   |   " + str(
                api_OpenWeather_json['wind']['speed']) + "KM/H   |   " + str(
                api_OpenWeather_json['clouds']['all']) + "% CC     "
...
if __name__ == "__main__":
    ...
    # THIS IS AN EMPTY VARIABLE FOR A REASON, Do not put anything in this variable!!!
disp_string_weatherInfo == ""
...
for number in range(150150150):
    ...
    sleep(1)
    # this is how many times it is going to repeat / how many news headers it will get (recommended 100)
    for i in range(100):
        ...
        sleep(1.5)
        # showing the date
        display.lcd_display_string(strftime('%w %-d %m %y ' '%H:%M '), 1)
        display.lcd_display_string(strftime('DAT2:%w %d %W %m '), 2)
        sleep(2.7)
        # showing UTC off-sync, timezone
        display.lcd_display_string(strftime('%w %-d %m %y ' '%H:%M '), 1)
        ...
        sleep(2.7)
        # showing the version, user, last updated and other info
        display.lcd_display_string(strftime('%w %-d %m %y ' '%H:%M '), 1)
        long_string(display, "www.vesbot.ca Hell even in my code I can advertise!", 2)
        sleep(2.7)
